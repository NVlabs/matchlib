...
#include "AxiSlaveToReadyValidTop.h"
#include <axi/AxiSlaveToReadyValid/testbench/RVSink.h>
#include <testbench/nvhls_rand.h>

SC_MODULE(testbench) {

  typedef AxiSlaveToReadyValidTop::axi_ axi_;
  typedef AxiSlaveToReadyValidTop::rdyvldCfg rdyvldCfg;
  struct masterCfg {
    enum {
      numWrites = 100,
      numReads = 100,
      readDelay = 200,  // Slave arbitrates reads and writes, so there's no
  ...
  SC_CTOR(testbench)
    ...
    slave.rv_read(rv_read);
    slave.rv_write(rv_write);
