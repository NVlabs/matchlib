...
struct nvhls_t {
  ...
  // conversion between ac to sc types
  template <int W>
  ac_int<W, true>& operator=(const sc_int<W>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  ac_int<W, true>& operator=(const sc_bigint<W>& that) {
    return that.to_ac();
  }
  template <int W>
  ac_int<W, false>& operator=(const sc_uint<W>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  ac_int<W, false>& operator=(const sc_biguint<W>& that) {
    return that.to_ac();
  }
  template <int W>
  ac_int<W, true>& operator=(const sc_uint<W>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  ac_int<W, true>& operator=(const sc_biguint<W>& that) {
    return that.to_ac();
  }
  template <int W>
  ac_int<W, false>& operator=(const sc_int<W>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  ac_int<W, false>& operator=(const sc_bigint<W>& that) {
    return that.to_ac();
  }

  template <int W>
  sc_int<W>& operator=(const ac_int<W, true>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  sc_bigint<W>& operator=(const ac_int<W, true>& that) {
    return that.to_ac();
  }
  template <int W>
  sc_uint<W>& operator=(const ac_int<W, false>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  sc_biguint<W>& operator=(const ac_int<W, false>& that) {
    return that.to_ac();
  }
  template <int W>
  sc_uint<W>& operator=(const ac_int<W, true>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  sc_biguint<W>& operator=(const ac_int<W, true>& that) {
    return that.to_ac();
  }
  template <int W>
  sc_int<W>& operator=(const ac_int<W, false>& that) {
    // sc_int width will be less than 64
    return that.to_int64();
  }
  template <int W>
  sc_bigint<W>& operator=(const ac_int<W, false>& that) {
    return that.to_ac();
  }
};
#else
