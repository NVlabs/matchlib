...
class FIFO<DataType, 0, NumBanks> {  // 0 entries, NumBanks
 ...
 public:
  ...
  void push(DataType wr_data, BankIdx bidx = 0) {NVHLS_ASSERT(0);}

  DataType pop(BankIdx bidx = 0) { NVHLS_ASSERT(0); return DataType(); }

  void incrHead(BankIdx bidx = 0) {NVHLS_ASSERT(0); }

  DataType peek(BankIdx bidx = 0) { NVHLS_ASSERT(0); return DataType(); }

  bool isEmpty(BankIdx bidx = 0) {NVHLS_ASSERT(0); return true; }

  bool isFull(BankIdx bidx = 0) {NVHLS_ASSERT(0); return true; }

  FifoIdx NumFilled(BankIdx bidx = 0) {NVHLS_ASSERT(0); return 0; }

  FifoIdx NumAvailable(BankIdx bidx = 0) {NVHLS_ASSERT(0); return 0; }
